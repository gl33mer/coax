...
use super::types::{CFG, VulnerabilitySlice, EntryPoint, SinkPoint, CfgFinding};
...
impl SliceIntersection {
    ...
    pub fn intersect(
        ...
        // Merge variables
        result.variables.extend(backward_slice.variables.iter().cloned());
        result.variables.extend(forward_slice.variables.iter().cloned());
    ...
    pub fn find_vulnerability_paths(
        ...
        let entry_nodes: Vec<NodeIndex> = entry_points.iter()
            .enumerate()
            .map(|(i, _)| {
                // Get node by index (simplified - real impl would map by name)
                cfg.graph.node_indices().nth(i % cfg.graph.node_count())
                    .unwrap_or(cfg.entry)
            })
            .collect();
        
        let sink_nodes: Vec<NodeIndex> = sink_points.iter()
            .enumerate()
            .map(|(i, _)| {
    ...
    pub fn filter_by_confidence(paths: Vec<VulnerabilitySlice>, min_confidence: f32) -> Vec<VulnerabilitySlice> {
        paths.into_iter()
    ...
    fn recommendation_from_sink(sink: &SinkPoint) -> String {
        match sink {
            ...
            SinkPoint::CommandExecution { .. } => {
                "Avoid executing commands with user input. Use allowlists and validate input".to_string()
            }
            ...
            SinkPoint::PathTraversal { .. } => {
                "Validate paths and use path canonicalization. Restrict to allowed directories".to_string()
            }
        }
    }
}
