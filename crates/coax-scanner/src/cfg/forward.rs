...
use super::types::{CFG, VulnerabilitySlice, EntryPoint, SinkPoint};
...
impl<'a> ForwardSlicer<'a> {
    ...
    pub fn slice(&self, entry_node: NodeIndex) -> VulnerabilitySlice {
        ...
        while let Some(node) = worklist.pop() {
            ...
            for succ in self.cfg.graph.neighbors_directed(node, petgraph::Direction::Outgoing) {
    ...
    pub fn slice_from_entry(&self, entry_node: NodeIndex, entry_point: EntryPoint) -> VulnerabilitySlice {
        let dummy_sink = SinkPoint::SqlExecution {
            query: "unknown".to_string(),
            method: "unknown".to_string(),
        };
